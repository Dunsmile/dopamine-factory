...
const CHECKLIST_REPORT_PATH = path.join(ROOT, 'reports', 'scaling-checklist-status.json');
const OUT_JSON = path.join(ROOT, 'reports', 'next-actions.json');
const OUT_MD = path.join(ROOT, 'reports', 'next-actions.md');
...
function mapAction(item) {
  ...
  if (text.includes('CTR +10%') || text.includes('시작률 +10%') || text.includes('완료율 +5')) {
    return {
      owner: 'Growth',
      input: 'reports/phaseb-kpi-input.json',
  ...
  if (text.includes('개발 생산성 +20%') || text.includes('운영 복잡도 증가 없음')) {
    return {
      owner: 'Platform',
      input: 'reports/astro-pilot-input.json',
  ...
  if (text.includes('Go 시 신규 1개 서비스만 별도 경로 파일럿')) {
    return {
      owner: 'Platform',
      input: 'reports/astro-pilot-execution.json',
  ...
  if (text.includes('리드타임 15분') || text.includes('실패율 0%') || text.includes('월 8개')) {
    return {
      owner: 'Ops',
      input: 'reports/phase-d-ops-input.json',

...
function loadQuestion(nextIndex = null) {
  const index = typeof nextIndex === 'number' ? nextIndex : Math.floor(Math.random() * BALANCE_QUESTIONS.length);
  ...
  document.getElementById('optionAButton').classList.remove('is-selected');
  document.getElementById('optionBButton').classList.remove('is-selected');
  document.getElementById('resultCard').classList.add('svc-hidden');
}
...
function updateResultView() {
  ...
  if (typeof window.trackServiceEvent === 'function') {
    window.trackServiceEvent('complete', { choice: selectedOption });
  }
}
...
function chooseOption(option) {
  ...
  document.getElementById('optionAButton').classList.toggle('is-selected', option === 'A');
  document.getElementById('optionBButton').classList.toggle('is-selected', option === 'B');
...
async function downloadBalanceShareCard() {
  ...
  await window.DopaminShareCard.download({
    ...
    tags: ['도파민선택', selectedOption === 'A' ? currentQuestion.optionA : currentQuestion.optionB],

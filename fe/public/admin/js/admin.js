...
(function initAdmin() {
  ...
  const PAGES = {
    ...
    services:   { title: '서비스',         render: renderServices },
    builder:    { title: '서비스 만들기',  render: renderBuilder },
    analytics:  { title: '애널리틱스',    render: renderAnalytics },
    sponsor:    { title: '스폰서',         render: renderSponsor },
  ...
  function navigate(page, params = {}) {
    ...
    const tabSuffix = (page === 'settings' && params.tab && params.tab !== 'general') ? `&tab=${params.tab}` : '';
    history.replaceState({ page, params }, '', `/admin/?page=${page}${tabSuffix}`);
  }
  ...
  function renderAnalytics() {
    ...
    contentEl.innerHTML = `
      ...
      <div class="adm-panel">
        <div class="adm-panel__header">
          <h2 class="adm-panel__title">애드센스 현황</h2>
        </div>
        <div class="adm-panel__body">
          <div style="display:grid;gap:10px;">
            <div style="padding:14px 16px;background:rgba(99,102,241,.06);border-radius:10px;border:1px solid rgba(99,102,241,.15);">
              <div style="font-size:12px;font-weight:700;color:var(--adm-accent);margin-bottom:4px;">ca-pub-7301223136166743</div>
              <div style="font-size:13px;color:var(--adm-text);">Auto Ads 설정 완료 — 모든 서비스 페이지 + 홈에 AdSense 스크립트 포함</div>
            </div>
            <div style="font-size:13px;color:var(--adm-text-muted);line-height:1.6;">
              수익 데이터는 <a href="https://www.google.com/adsense" target="_blank" rel="noopener" style="color:var(--adm-accent);">Google AdSense 대시보드</a>에서 확인하세요.<br>
              월 목표: 100만원 · 현재 단계: 승인 대기 or 트래픽 확보 중
            </div>
          </div>
        </div>
      </div>
...
&lt;/noscript&gt;</pre>
  ...
  loadData().then(() => {
    ...
    let initPage = urlParams.get('page') || 'dashboard';
    const initId  = urlParams.get('id');
    const initTab = urlParams.get('tab');

    // 구버전 URL 호환: categories/home → settings with tab
    if (initPage === 'categories') { navigate('settings', { tab: 'categories' }); return; }
    if (initPage === 'home')       { navigate('settings', { tab: 'home' });       return; }

    if (initPage === 'services' && initId) {
      navigate('services');
      setTimeout(() => window.__admOpenDetail(initId), 100);
    } else if (initTab && initPage === 'settings') {
      navigate('settings', { tab: initTab });
    } else {
